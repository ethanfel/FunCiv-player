import { clipRating, isDraftClip } from '../../packages/composer-core/index.mjs';
const esc=value=>String(value??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
const stamp=ms=>`${Math.floor(ms/60000)}:${(ms/1000%60).toFixed(1).padStart(4,'0')}`;
const stars=n=>n?`${n}★`:'Unrated';
const ready=c=>c.available&&c.script_ready;
...
export class ClipLibrary {
  ...
  render(clips,used,minimum,empty){
    ...
    list.innerHTML=clips.map(c=>{
      const rating=clipRating(c),status=ready(c)?'Ready':c.origin==='dataset'?(c.available?'Get scripts':'Download'):c.available?'Video only':'Missing';
      return `<article class="fc-clip ${c.id===this.selected?'fc-selected':''}" data-clip-id="${esc(c.id)}" data-rating="${rating}"><button class="fc-clip-row" data-action="inspect-library-clip" data-id="${esc(c.id)}" aria-pressed="${c.id===this.selected}" title="${esc(c.name)}"><span class="fc-clip-heading"><strong>${esc(c.name)}</strong><time>${stamp(c.duration_ms)}</time></span><span class="fc-clip-meta"><span class="fc-clip-stars">${stars(rating)}</span><span class="fc-availability ${ready(c)?'fc-ready':''}">${status}</span>${isDraftClip(c)?'<span class="fc-draft-badge">Draft</span>':''}${used.has(c.id)?`<span>Used ${used.get(c.id)}×</span>`:''}</span><span class="fc-clip-category">${esc((c.categories||[]).join(', ')||'Uncategorized')}${rating<minimum?' · Below minimum':''}${!v.includeDrafts&&isDraftClip(c)?' · Draft excluded':''}</span></button></article>`;
    ...
    details.innerHTML=`<div class="fc-panel-heading"><h3>Selected clip</h3><span>${isDraftClip(clip)?'Draft · unreviewed':esc(clip.review_status||'Local')}</span></div><strong class="fc-detail-title">${esc(clip.name)}</strong>
      ...
      ${clip.origin==='dataset'?`<button data-action="resolve" data-id="${esc(clip.id)}" class="${ready(clip)?'':'fc-primary'}">${ready(clip)?'Verify / refresh':'Resolve video + scripts'}</button>`:''}`;
  }
}
